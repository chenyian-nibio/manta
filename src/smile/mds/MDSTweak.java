...
/**
 * This is a tweak version of MDS in SMILE library developed by Haifeng Li <br/><br/>
 * 
 ...
 * @author Haifeng Li
 */
public class MDSTweak {
    private static final Logger logger = LoggerFactory.getLogger(MDSTweak.class);
     ...
     */
    public MDSTweak(double[][] proximity) {
        this(proximity, 2);
    }
     ...
     */
    public MDSTweak(double[][] proximity, int k) {
     ...
     */
    public MDSTweak(double[][] proximity, int k, boolean add) {
        ...
        // collect only the positive eigen values
        int p = 0;
        for (double e: eigen.getEigenValues()) {
        	if (e > 0) {
        		p++;
        	}
        }
        eigenvalues = Arrays.copyOf(eigen.getEigenValues(), p);
        
        if (eigenvalues.length < k) {
            logger.warn("eigen({}) returns only {} eigen vectors", k, eigenvalues.length);
